...
{
    makeNParticlesI(ptxAtlas, 256,
        [&](const int i, PHandle p)
        {
            p.atlasIdx() = aiCircle;
            p.angle() = rndToRad(0.f, 360.f);
            p.setAlpha(255);
    ...
    const qvec3& org, const qvec3& dir, int color, int count)
{
    ...
    makeNParticles(ptxAtlas, debrisCount,
        [&](PHandle p)
        {
            p.atlasIdx() = aiRock;
            p.angle() = rndToRad(0.f, 360.f);
    ...
    makeNParticles(ptxAtlas, dustCount,
        [&](PHandle p)
        {
            p.atlasIdx() = aiCircle;
            p.angle() = rndToRad(0.f, 360.f);
    ...
    makeNParticles(ptxAtlas, sparkCount,
        [&](PHandle p)
        {
            p.atlasIdx() = aiSpark;
            p.angle() = rndToRad(0.f, 360.f);
...
void R_RunParticleEffect_Blood(const qvec3& org, const qvec3& dir, int count)
{
    ...
    makeNParticles(ptxAtlas, count * 2,
        [&](PHandle p)
        {
            p.atlasIdx() = aiBlood;
            p.angle() = rndToRad(0.f, 360.f);
    ...
    makeNParticles(ptxAtlas, count * 24,
        [&](PHandle p)
        {
            p.atlasIdx() = aiCircle;
            p.angle() = rndToRad(0.f, 360.f);
    ...
    const qvec3& org, const qvec3& dir, int count)
{
    ...
    makeNParticles(ptxAtlas, count,
        [&](PHandle p)
        {
            ...
            {
                p.org()[j] =
                    org[j] + static_cast<float>(((rand() & 2) - 1)) * 0.3f;
                p.vel()[j] = rnd(-2, 2);
            }
        });
}
...
void CL_RunParticles()
{
    ...
    pMgr.forBuffers(
        [&](gltexture_t*, const ImageData&, PBuffer& pBuffer)
        {
        ...
        [&](PHandle p)
        {
            switch(p.type())
            {
                ...
                case pt_txexplode:
                {
                    ...
                    p.angle() +=
                        0.75f * frametime * (p.param0() == 0 ? 1.f : -1.f);
                ...
                {
                    p.angle() +=
                        25.f * frametime * (p.param0() == 0 ? 1.f : -1.f);
...
void R_DrawParticles()
{
    ...
    pMgr.forBuffers(
        [&](gltexture_t* texture, const ImageData& imageData, PBuffer& pBuffer)
        {
            (void)texture;
            (void)imageData;
            ...
            glBufferData(GL_ARRAY_BUFFER, sizeof(qvec3) * pCount, soa._orgs,
                GL_STATIC_DRAW);
