...
Audio resampling functions:   Originally from DarkPlaces, presumably LordHavoc
is the original author. Raw Audio Capture (DSound):   Spike. Networking: Spike.
...
Servers send a 'cmd pext\n' stufftext. The client intercepts this and replaces
it with a 'cmd pext KEY VALUE ...\n' command. When the server receives the
extended pext command, it reads the key+value pairs, and tells the client the
extensions that be used for the connection. No voice commands nor packets will
be sent which the other is unable to understand. The exception is in the form of
demos. Such demos will require the replaying client to understand the data.
...
#ifndef _WIN32 // for some reason, the win32 libs lack opus_encode* symbols. the
               // dlls contain them though. I've no idea what's going on there.
#define OPUS_STATIC
#endif
#endif
...
{
    Con_SafePrintf(
        "Sys_LoadLibrary(%s) is not implemented on this platform\n", name);
    return nullptr;
}
#endif
...
typedef struct
{
    ...
    void (*Start)(
        void* ctx); /*begin grabbing new data, old data is potentially flushed*/
    unsigned int (*Update)(void* ctx, unsigned char* buffer,
        unsigned int minbytes,
        unsigned int maxbytes);  /*grab the data into a different buffer*/
...
#define USE_SDL_CAPTURE 1

#ifdef USE_DSOUND_CAPTURE
#include <mmsystem.h>
#include <dsound.h>
static HINSTANCE hInstDSC;
static HRESULT(WINAPI* pDirectSoundCaptureCreate)(GUID FAR* lpGUID,
    LPDIRECTSOUNDCAPTURE FAR* lplpDS, IUnknown FAR* pUnkOuter);
...
static void* DSOUND_Capture_Init(int rate)
{
    ...
    wfxFormat.nBlockAlign =
        wfxFormat.nChannels * (wfxFormat.wBitsPerSample / 8);
    wfxFormat.nAvgBytesPerSec =
        wfxFormat.nSamplesPerSec * wfxFormat.nBlockAlign;
    wfxFormat.cbSize = 0;
    ...
    if(!pDirectSoundCaptureCreate)
    {
        pDirectSoundCaptureCreate =
            (void*)GetProcAddress(hInstDSC, "DirectSoundCaptureCreate");
        ...
        //		pDirectSoundCaptureEnumerate = (void
        //*)GetProcAddress(hInstDS,"DirectSoundCaptureEnumerateA");
    }

    result = (dsndcapture_t*)Z_Malloc(sizeof(*result));
    if(!FAILED(pDirectSoundCaptureCreate(nullptr, &result->DSCapture, nullptr)))
    {
        if(!FAILED(IDirectSoundCapture_CreateCaptureBuffer(
               result->DSCapture, &bufdesc, &result->DSCaptureBuffer, nullptr)))
        {
            return result;
        }
...
static unsigned int DSOUND_Capture_Update(void* ctx, unsigned char* buffer,
    unsigned int minbytes, unsigned int maxbytes)
{
    auto* c = (dsndcapture_t*)ctx;
    HRESULT hr;
    ...
    hr = IDirectSoundCaptureBuffer_GetCurrentPosition(
        c->DSCaptureBuffer, &capturePos, &readPos);
    if(hr != DS_OK)
    {
        return 0;
    }
    ...
    if(filled > (long)maxbytes)
    { // figure out how much we need to empty it by,
        // and if that's enough to be worthwhile.
        filled = maxbytes;
    }
    ...
    hr = IDirectSoundCaptureBuffer_Lock(c->DSCaptureBuffer, c->lastreadpos,
        filled, (void**)&lpbuf1, &dwsize1, (void**)&lpbuf2, &dwsize2, 0);
    if(hr == DS_OK)
    {
        ...
        IDirectSoundCaptureBuffer_Unlock(
            c->DSCaptureBuffer, lpbuf1, dwsize1, lpbuf2, dwsize2);
    }
    else
    {
        return 0;
...
static snd_capture_driver_t DSOUND_Capture = {DSOUND_Capture_Init,
    DSOUND_Capture_Start, DSOUND_Capture_Update, DSOUND_Capture_Stop,
    DSOUND_Capture_Shutdown};
#endif
#ifdef USE_SDL_CAPTURE
// Requires SDL 2.0.5+ supposedly.
// Bugging out for me on windows, with really low audio levels. looks like
// there's been some float->int conversion without a multiplier. asking for
// float audio gives stupidly low values too.
typedef struct
{
...
static void* SDL_Capture_Init(int rate)
{
    ...
    want.channels = 1;
    want.samples =
        256; // this seems to be chunk sizes rather than total buffer size, so
             // lets keep it reasonably small for lower latencies
    want.callback = nullptr;

    c.dev = SDL_OpenAudioDevice(nullptr, true, &want, &have, 0);
    if(!c.dev) // failed?
        return nullptr;

    r = (sdlcapture_t*)Z_Malloc(sizeof(*r));
    *r = c;
    return r;
}

/*minbytes is a hint to not bother wasting time*/
static unsigned int SDL_Capture_Update(void* ctx, unsigned char* buffer,
    unsigned int minbytes, unsigned int maxbytes)
{
    auto* c = (sdlcapture_t*)ctx;
    unsigned int queuedsize = SDL_GetQueuedAudioSize(c->dev);
    if(queuedsize < minbytes) return 0;
    if(queuedsize > maxbytes) queuedsize = maxbytes;

    queuedsize = SDL_DequeueAudio(c->dev, buffer, queuedsize);
    return queuedsize;
}
static snd_capture_driver_t SDL_Capture = {SDL_Capture_Init, SDL_Capture_Start,
    SDL_Capture_Update, SDL_Capture_Stop, SDL_Capture_Shutdown};
#endif
...
#define LINEARUPSCALE(in, inrate, insamps, out, outrate, outlshift, outrshift) \
    {                                                                          \
        ...
        while(outsamps)                                                        \
        {                                                                      \
            *out = ((0xFFFF - inaccum) * in[0] + inaccum * in[1]) >>           \
                   (16 - outlshift + outrshift);                               \
            inaccum += infrac;                                                 \
            in += (inaccum >> 16);                                             \
            inaccum &= 0xFFFF;                                                 \
            out++;                                                             \
...
#define LINEARUPSCALESTEREO(                                           \
    in, inrate, insamps, out, outrate, outlshift, outrshift)           \
    {                                                                  \
        scale = inrate / (double)outrate;                              \
        infrac = floor(scale * 65536);                                 \
        outsamps = insamps / scale;                                    \
        ...
        while(outsamps)                                                \
        {                                                              \
            out[0] = ((0xFFFF - inaccum) * in[0] + inaccum * in[2]) >> \
                     (16 - outlshift + outrshift);                     \
            out[1] = ((0xFFFF - inaccum) * in[1] + inaccum * in[3]) >> \
                     (16 - outlshift + outrshift);                     \
            inaccum += infrac;                                         \
            in += (inaccum >> 16) * 2;                                 \
            inaccum &= 0xFFFF;                                         \
...
#define LINEARUPSCALESTEREOTOMONO(                                        \
    in, inrate, insamps, out, outrate, outlshift, outrshift)              \
    {                                                                     \
        scale = inrate / (double)outrate;                                 \
        infrac = floor(scale * 65536);                                    \
        ...
        while(outsamps)                                                   \
        {                                                                 \
            *out = ((((0xFFFF - inaccum) * in[0] + inaccum * in[2]) >>    \
                        (16 - outlshift + outrshift)) +                   \
                       (((0xFFFF - inaccum) * in[1] + inaccum * in[3]) >> \
                           (16 - outlshift + outrshift))) >>              \
                   1;                                                     \
            inaccum += infrac;                                            \
            in += (inaccum >> 16) * 2;                                    \
...
#define LINEARDOWNSCALESTEREOTOMONO(                                        \
    in, inrate, insamps, out, outrate, outlshift, outrshift)                \
    {                                                                       \
        scale = outrate / (double)inrate;                                   \
        infrac = floor(scale * 65536);                                      \
...
#define STANDARDRESCALESTEREOTOMONO(                             \
    in, inrate, insamps, out, outrate, outlshift, outrshift)     \
    {                                                            \
        scale = inrate / (double)outrate;                        \
        infrac = floor(scale * 65536);                           \
...
// SND_ResampleStream: takes a sound stream and converts with given parameters.
// Limited to 8-16-bit signed conversions and mono-to-mono/stereo-to-stereo
// conversions. Not an in-place algorithm.
void SND_ResampleStream(void* in, int inrate, int inwidth, int inchannels,
    int insamps, void* out, int outrate, int outwidth, int outchannels,
    int resampstyle)
{
    double scale;
        ...
        if(inwidth == 1)
        {
            if(outwidth == 1)
            {
                if(inrate < outrate) // upsample
                {
                    if(resampstyle)
                        ...
                        STANDARDRESCALE(
                            in8, inrate, insamps, out8, outrate, 0, 0)
                }
                else // downsample
                {
                    if(resampstyle > 1)
            ...
            else
            {
                ...
                else if(inrate < outrate) // upsample
                {
                    if(resampstyle)
                        LINEARUPSCALE(
                            in8, inrate, insamps, out16, outrate, 8, 0)
                    else
                        STANDARDRESCALE(
                            in8, inrate, insamps, out16, outrate, 8, 0)
                }
        ...
        else // 16-bit
        {
            if(outwidth == 2)
            {
                if(inrate < outrate) // upsample
                {
                    if(resampstyle)
                        LINEARUPSCALE(
                            in16, inrate, insamps, out16, outrate, 0, 0)
                    else
                        STANDARDRESCALE(
                            in16, inrate, insamps, out16, outrate, 0, 0)
                }
            ...
            else
            {
                ...
                else if(inrate < outrate) // upsample
                {
                    if(resampstyle)
                        LINEARUPSCALE(
                            in16, inrate, insamps, out8, outrate, 0, 8)
                    else
                        STANDARDRESCALE(
                            in16, inrate, insamps, out8, outrate, 0, 8)
                }
            ...
            if(outwidth == 1)
            {
                if(inrate < outrate) // upsample
                {
                    if(resampstyle)
                        ...
                        STANDARDRESCALESTEREO(
                            in8, inrate, insamps, out8, outrate, 0, 0)
                }
                else // downsample
                {
                    if(resampstyle > 1)
                        LINEARDOWNSCALESTEREO(
                            in8, inrate, insamps, out8, outrate, 0, 0)
                    else
                        STANDARDRESCALESTEREO(
                            in8, inrate, insamps, out8, outrate, 0, 0)
...
#define MAX_RAW_CACHE \
    (1024 * 32) // have no idea which size it actually should be. this specifies
                // the maximum buffer size. if it gets full, the entire thing is
                // dumped in order to try to reduce latency (we have no time
                // drifting or anything)
...
// Streaming audio.
// This is useful when there is one source, and the sound is to be played with
// no attenuation.
void S_RawAudio(int sourceid, byte* data, unsigned int speed,
    unsigned int samples, unsigned int channelsnum, unsigned int width,
    float volume)
{
    int i;
    int newsize;
    ...
    if(!s->inuse)
    {
        ...
        newcache = (sfxcache_t*)Cache_Alloc(&s->sfx.cache, newsize, "rawaudio");
        if(!newcache)
        {
        ...
        s->inuse = true;
        s->id = sourceid;
        // strcpy(s->sfx.name, ""); // FIXME: probably we should put some
        // specific tag name here?
    ...
    // prepadl is the length of data at the start of the sample which appears to
    // be unused. its the amount of data that we can discard.
    prepadl = 0x7fffffff;
    for(i = 0; i < total_channels; i++)
    {
        if(snd_channels[i].sfx == &s->sfx)
    ...
    else
    {
        ...
        if(spare < 0)
        { // remaining samples since last time
            spare = 0;
        }
    ...
    newsize =
        (spare + outsamples) * (currentcache->stereo + 1) * currentcache->width;
    if(newsize > MAX_RAW_CACHE)
    { // this can happen quite often when our playback driver isn't playing
      // sound due to the window not having focus.
    ...
    memmove(currentcache->data,
        currentcache->data +
            prepadl * (currentcache->stereo + 1) * currentcache->width,
    ...
    {
        short* outpos =
            (short*)(currentcache->data +
                     spare * (currentcache->stereo + 1) * currentcache->width);
        SND_ResampleStream(data, speed, width, channelsnum, samples, outpos,
            shm->speed, currentcache->width, currentcache->stereo + 1, true);
    }
    ...
    for(i = 0; i < total_channels; i++)
    {
        if(snd_channels[i].sfx == &s->sfx)
        {
            ...
            snd_channels[i].master_vol =
                (int)(volume * 255); // this should changed volume on alredy
                                     // playing sound.
...
cvar_t cl_voip_codec = {
    "cl_voip_codec", "", true}; // opus by default (quakespasm actually comes
                                // with a dll for that one)
...
{
    VOIP_SPEEX_OLD =
        0, // original supported codec (with needless padding [because I didn't
           // understand speex's format] narrowband speex encoded+played at the
           // wrong rate [because it was easier at the time])
    VOIP_RAW16 = 1, // support is not recommended, but useful for testing.
                    // 16bit, 11khz, mono
    ...
    VOIP_PCMA =
        6, // G711 is kinda shit, encoding audio at 8khz with funny
           // truncation for 13bit to 8bit, but its quite simple, and
           // smaller than raw pcm, as well as a well-known voip standard
...
static struct
{
    ...
    unsigned char decseq[MAX_SCOREBOARD]; /*sender's sequence, to detect+cover
                                             minor packetloss*/
    unsigned char decgen[MAX_SCOREBOARD]; /*last generation. if it changes, we
                                             flush speex to reset packet loss*/
    unsigned int decsamplerate[MAX_SCOREBOARD];
    unsigned int decframesize[MAX_SCOREBOARD];
    float lastspoke[MAX_SCOREBOARD]; /*time when they're no longer considered
                                        talking. if future, they're talking*/
    float lastspoke_any;
    ...
    bool wantsend;           /*set if we're capturing data to send*/
...
static OpusEncoder*(QDECL* qopus_encoder_create)(
...
static opus_int32(QDECL* qopus_encode)(OpusEncoder* st, const opus_int16* pcm,
    int frame_size, unsigned char* data, opus_int32 max_data_bytes);
static OpusDecoder*(QDECL* qopus_decoder_create)(
...
static int(QDECL* qopus_decode)(OpusDecoder* st, const unsigned char* data,
...
static int(QDECL* qspeex_encode_int)(
    void* state, spx_int16_t* in, SpeexBits* bits);

static void*(QDECL* qspeex_decoder_init)(const SpeexMode* mode);
static void(QDECL* qspeex_decoder_destroy)(void* state);
static int(QDECL* qspeex_decode_int)(
    void* state, SpeexBits* bits, spx_int16_t* out);
static void(QDECL* qspeex_bits_read_from)(
    SpeexBits* bits, char* bytes, int len);
...
#ifdef USE_SPEEX_DSP
static SpeexPreprocessState*(QDECL* qspeex_preprocess_state_init)(
    int frame_size, int sampling_rate);
static void(QDECL* qspeex_preprocess_state_destroy)(SpeexPreprocessState* st);
static int(QDECL* qspeex_preprocess_ctl)(
    SpeexPreprocessState* st, int request, void* ptr);
static int(QDECL* qspeex_preprocess_run)(
    SpeexPreprocessState* st, spx_int16_t* x);
...
#ifdef USE_SPEEX_DSP
static bool S_SpeexDSP_Init()
{
#ifndef SPEEX_STATIC
    if(s_voip.speexdsp.inited) return s_voip.speexdsp.loaded;
    s_voip.speexdsp.inited = true;


    s_voip.speexdsp.speexdsplib =
        Sys_LoadLibrary("libspeexdsp", qspeexdspfuncs);
...
#ifdef USE_SPEEX_CODEC
static bool S_Speex_Init()
{
#ifndef SPEEX_STATIC
    if(s_voip.speex.inited) return s_voip.speex.loaded;
...
static bool S_Opus_Init(int encdec)
{
#ifndef OPUS_STATIC
#ifdef _WIN32
    const char* modulename = "libopus-0.dll";
//	const char *altmodulename = "libopus.dll";
#else
    const char* modulename = "libopus.so.0";
//	const char *altmodulename = "libopus.so";
#endif
    ...
    s_voip.opus.opuslib[encdec] = Sys_LoadLibrary(
        modulename, encdec ? qopusencodefuncs : qopusdecodefuncs);
    //	if (!s_voip.opus.opuslib[encdec] && encdec)
    //		s_voip.opus.opuslib[encdec] = Sys_LoadLibrary(altmodulename,
    // encdec?qopusencodefuncs:qopusdecodefuncs);
    if(!s_voip.opus.opuslib[encdec])
    {
        Con_Printf(
            "%s or its exports not found. Opus voip %s is not available.\n",
...
// there's two forms, a-law is generally considered better quality, u-law is a
// little simpler.
...
static size_t PCMA_Encode(
    unsigned char* out, size_t outsize, short* in, size_t samples)
{
    size_t i = 0;
    for(i = 0; i < samples; i++)
    {
...
void S_Voip_Decode(unsigned int sender, unsigned int codec, unsigned int gen,
    unsigned char seq, unsigned int bytes, unsigned char* data)
{
    unsigned char* start;
    ...
    // if they re-started speaking, flush any old state to avoid things getting
    // weirdly delayed and reset the codec properly.
    if(s_voip.decgen[sender] != gen || s_voip.deccodec[sender] != codec)
    {
        S_RawAudio(sender, nullptr, s_voip.decsamplerate[sender], 0, 1, 2, 0);
        ...
        switch(codec)
        {
            default: // codec not supported.
                return;
            case VOIP_RAW16: s_voip.decsamplerate[sender] = 11025; break;
            case VOIP_PCMA:
            case VOIP_PCMU:
            ...
            case VOIP_SPEEX_ULTRAWIDE:
            {
                ...
                if(!S_Speex_Init()) return; // speex not usable.
                ...
                if(!s_voip.decoder[sender])
                {
                    ...
                    if(!s_voip.decoder[sender]) return;
                }
                else
                    qspeex_bits_reset(&s_voip.speex.decbits[sender]);
            }
            ...
            case VOIP_OPUS:
                ...
                if(!s_voip.decoder[sender])
                {
                    // opus outputs to 8, 12, 16, 24, or 48khz. pick whichever
                    // has least excess samples and resample to fit it.
                    if(shm->speed <= 8000)
                    {
                        s_voip.decsamplerate[sender] = 8000;
                    }
                    ...
                    s_voip.decoder[sender] = qopus_decoder_create(
                        s_voip.decsamplerate[sender],
                        1 /*FIXME: support stereo where possible*/, nullptr);
                    if(!s_voip.decoder[sender])
                    {
                        return;
                    }

                    s_voip.decframesize[sender] =
                        s_voip.decsamplerate[sender] /
                        400; // this is the maximum size in a single frame.
                }
                else
                {
                    qopus_decoder_ctl(
    ...
    {
        if(decodesamps + s_voip.decframesize[sender] >
            sizeof(decodebuf) / sizeof(decodebuf[0]))
        {
            S_RawAudio(sender, (byte*)decodebuf, s_voip.decsamplerate[sender],
                decodesamps, 1, 2, cl_voip_play.value);
            decodesamps = 0;
        }
        switch(codec)
        {
            case VOIP_RAW16: break; // just skip it.
#ifdef USE_SPEEX_CODEC
            case VOIP_SPEEX_OLD:
            case VOIP_SPEEX_NARROW:
            case VOIP_SPEEX_WIDE:
            case VOIP_SPEEX_ULTRAWIDE:
                qspeex_decode_int(
                    s_voip.decoder[sender], nullptr, decodebuf + decodesamps);
    ...
    while(bytes > 0)
    {
        ...
        switch(codec)
        {
            ...
            case VOIP_SPEEX_ULTRAWIDE:
                ...
                { // older versions support only this, and require this extra
                  // bit.
                    bytes--;
                    len = *start++;
                    if(bytes < len) break;
                    ...
                    len = bytes;
                qspeex_bits_read_from(
                    &s_voip.speex.decbits[sender], start, len);
                bytes -= len;
                start += len;
                while(qspeex_decode_int(s_voip.decoder[sender],
                          &s_voip.speex.decbits[sender],
                          decodebuf + decodesamps) == 0)
                {
                    decodesamps += s_voip.decframesize[sender];
                    s_voip.decseq[sender]++;
                    seq++;
                    if(decodesamps + s_voip.decframesize[sender] >=
                        sizeof(decodebuf) / sizeof(decodebuf[0]))
                    {
                        S_RawAudio(sender, (byte*)decodebuf,
                            s_voip.decsamplerate[sender], decodesamps, 1, 2,
                            cl_voip_play.value);
                        decodesamps = 0;
                    }
                }
            ...
            case VOIP_PCMU:
                ...
                {
                    decodesamps +=
                        PCMA_Decode(decodebuf + decodesamps, start, len);
                }
                else
                {
                    decodesamps +=
                        PCMU_Decode(decodebuf + decodesamps, start, len);
                }
                s_voip.decseq[sender]++;
                bytes -= len;
                start += len;
            ...
            case VOIP_OPUS:
                ...
                if(r > 0)
                {
                    ...
                    decodesamps += r;
                    s_voip.decseq[sender] =
                        (s_voip.decseq[sender] + frames) & 0xff;
                    seq = (seq + frames) & 0xff;
                }
                else if(r < 0)
                {
...
void S_Voip_Parse()
{
    ...
    {
        while(bytes-- > 0) (void)MSG_ReadByte();
        return;
    }
    for(i = 0; i < bytes; i++) data[i] = MSG_ReadByte();
    ...
    if(cl_voip_test.value)
        if((int)sender ==
            cl.viewentity - 1) // FIXME: this isn't exactly reliable
            return;
    ...
    for(i = 0; i < 16; i++)
    {
        switch(i)
        {
            ...
            case VOIP_OPUS:
                codecname = "Opus";
                codecsupported =
                    (S_Opus_Init(false) ? 1 : 0) | (S_Opus_Init(true) ? 2 : 0);
                break;
            default:
                continue;
...
void S_Voip_EncodeStop()
{
    if(s_voip.cdriver)
    {
        if(s_voip.cdriverctx)
        {
            ...
            s_voip.cdriverctx = nullptr;
        }
        s_voip.cdriver = nullptr;
    }
...
void S_Voip_Transmit(unsigned char clc, sizebuf_t* buf)
{
...
#ifdef USE_SPEEX_DSP
    static bool cl_voip_noisefilter_old = 0;
    static bool cl_voip_autogain_old = 0;
#endif
    ...
    if(buf)
    {
        /*if you're sending sound, you should be prepared to accept others
         * yelling at you to shut up*/
    ...
    voicevolumescale =
        s_voip.lastspoke_any > realtime ? cl_voip_ducking.value : 1;
    ...
    { // empty cvar = pick a default that should work...
        if(S_Opus_Init(true)) voipcodec = VOIP_OPUS;
#ifdef USE_SPEEX_CODEC
    ...
    if(!s_voip.cdriver && voipsendenable)
    {
        ...
        switch(voipcodec)
        {
            ...
            case VOIP_SPEEX_ULTRAWIDE:
            {
                ...
                if(!s_voip.encoder) return;
                qspeex_encoder_ctl(
                    s_voip.encoder, SPEEX_GET_FRAME_SIZE, &s_voip.encframesize);
                qspeex_encoder_ctl(s_voip.encoder, SPEEX_GET_SAMPLING_RATE,
                    &s_voip.encsamplerate);
                ...
                qspeex_encoder_ctl(s_voip.encoder, SPEEX_SET_SAMPLING_RATE,
                    &s_voip.encsamplerate);
            }
            break;
#endif
            ...
            case VOIP_OPUS:
                ...
                s_voip.encframesize =
                    s_voip.encsamplerate / 400; // 2.5ms frames, at a minimum.
                s_voip.encoder = qopus_encoder_create(
                    s_voip.encsamplerate, 1, OPUS_APPLICATION_VOIP, nullptr);
                if(!s_voip.encoder)
                {
                    return;
                }
                ...
                // OPUS_SET_BITRATE(bitrate_bps)); opus_encoder_ctl(enc,
                // OPUS_SET_BANDWIDTH(OPUS_BANDWIDTH_NARROWBAND));
                //			opus_encoder_ctl(enc, OPUS_SET_VBR(use_vbr));
                //			opus_encoder_ctl(enc,
                // OPUS_SET_VBR_CONSTRAINT(cvbr)); opus_encoder_ctl(enc,
                // OPUS_SET_COMPLEXITY(complexity));
                // opus_encoder_ctl(enc, OPUS_SET_INBAND_FEC(use_inbandfec));
                // opus_encoder_ctl(enc,
                // OPUS_SET_FORCE_CHANNELS(forcechannels));
                // opus_encoder_ctl(enc, OPUS_SET_DTX(use_dtx));	//FIXME: we
                // should probably use this one, as we're sending network
                // packets regardless. 			opus_encoder_ctl(enc,
        ...
        if(!s_voip.cdriverctx) Con_Printf("No microphone detected\n");
    }
    ...
    {
        s_voip.wantsend = false;
        s_voip.capturepos += s_voip.cdriver->Update(s_voip.cdriverctx,
            (unsigned char*)s_voip.capturebuf + s_voip.capturepos, 1,
            sizeof(s_voip.capturebuf) - s_voip.capturepos);
        s_voip.cdriver->Stop(s_voip.cdriverctx);
        /*note: we still grab audio to flush everything that was captured while
         * it was active*/
    }
    else if(voipsendenable && !s_voip.wantsend)
    {
        ...
        { /*if we were actually still sending, it was probably only off for a
             single frame, in which case don't reset it*/
            s_voip.dumps = 0;
            s_voip.generation++;
            s_voip.encsequence = 0;

            // reset codecs so they start with a clean slate when new audio
    ...
    s_voip.capturepos += s_voip.cdriver->Update(s_voip.cdriverctx,
        (unsigned char*)s_voip.capturebuf + s_voip.capturepos,
        s_voip.encframesize * 2, sizeof(s_voip.capturebuf) - s_voip.capturepos);
    ...
    for(encpos = 0, outpos = 0;
        s_voip.capturepos - encpos >= s_voip.encframesize * 2 &&
        sizeof(outbuf) - outpos > 64;)
    {
        ...
        {
            if(!s_voip.speexdsp.preproc ||
                cl_voip_noisefilter.value != cl_voip_noisefilter_old ||
                cl_voip_autogain.value != cl_voip_autogain_old ||
                ...
                s_voip.speexdsp.cursamplerate != s_voip.encsamplerate)
            {
                ...
                s_voip.speexdsp.preproc = nullptr;
                if(S_SpeexDSP_Init())
                {
                    int i;
                    s_voip.speexdsp.preproc = qspeex_preprocess_state_init(
                        s_voip.encframesize, s_voip.encsamplerate);
                    i = cl_voip_noisefilter.value;
                    qspeex_preprocess_ctl(s_voip.speexdsp.preproc,
                        SPEEX_PREPROCESS_SET_DENOISE, &i);
                    i = cl_voip_autogain.value;
                    qspeex_preprocess_ctl(
                        s_voip.speexdsp.preproc, SPEEX_PREPROCESS_SET_AGC, &i);
        ...
        switch(s_voip.enccodec)
        {
            ...
            case VOIP_SPEEX_OLD:
                ...
                len = qspeex_bits_write(&s_voip.speex.encbits,
                    outbuf + (outpos + 1), sizeof(outbuf) - (outpos + 1));
            ...
            case VOIP_SPEEX_ULTRAWIDE:
                qspeex_bits_reset(&s_voip.speex.encbits);
                for(; s_voip.capturepos - encpos >= s_voip.encframesize * 2 &&
                      sizeof(outbuf) - outpos > 64;)
                {
                    start = (short*)(s_voip.capturebuf + encpos);
                    level +=
                        S_Voip_Preprocess(start, s_voip.encframesize, micamp);
                    qspeex_encode_int(
                        s_voip.encoder, start, &s_voip.speex.encbits);
                    s_voip.encsequence++;
                    samps += s_voip.encframesize;
                    encpos += s_voip.encframesize * 2;
                }
                len = qspeex_bits_write(&s_voip.speex.encbits, outbuf + outpos,
                    sizeof(outbuf) - outpos);
                outpos += len;
                break;
#endif
            case VOIP_RAW16:
                len = s_voip.capturepos -
                      encpos; // amount of data to be eaten in this frame
                ...
                s_voip.encsequence++; // increment number of packets, for
                                      // packetloss detection.
                samps +=
                    len /
                    2; // number of samplepairs eaten in this packet. for stats.
                break;
            case VOIP_PCMA:
            case VOIP_PCMU:
                len = s_voip.capturepos -
                ...
                {
                    outpos += PCMA_Encode(outbuf + outpos,
                        sizeof(outbuf) - outpos, start, len / 2);
                }
                else
                {
                    outpos += PCMU_Encode(outbuf + outpos,
                        sizeof(outbuf) - outpos, start, len / 2);
                }
                encpos += len;        // number of bytes consumed
                s_voip.encsequence++; // increment number of packets, for
            ...
            case VOIP_OPUS:
            {
                ...
                else if(frames >= 960)
                {
                    ...
                    break; // don't generate small frames if we're recording a
                           // demo, due to overheads.
                else if(frames >= 480)
                {
                    frames = 480;
                }
                ...
                if(nrate != s_voip.curbitrate)
                {
                    ...
                    { //"Rates from 500 to 512000 bits per second are
                      // meaningful"
    ...
    if(samps)
    {
        ...
        if(s_voip.voiplevel < cl_voip_vad_threshhold.value &&
            !((int)cl_voip_send.value & 6) && !s_voip.voipsendbutton)
        {
            /*try and dump it, it was too quiet, and they're not pressing
             * +voip*/
    ...
    if(outpos)
    {
        ...
        if(cl_voip_send.value != 4)
        {
            ...
            if(buf && (unsigned int)(buf->maxsize - buf->cursize) >= outpos + 5)
            {
                ...
                MSG_WriteByte(buf,
                    (s_voip.enccodec << 4) |
                        (s_voip.generation &
                            0x0f)); /*gonna leave that nibble clear here... in
                                       this version, the client will ignore
                                       packets with those bits set. can use them
                                       for codec or something*/
            ...
            if(cls.demorecording &&
                (!sv.active ||
                    !sv_voip_echo.value)) // voice is not (normally) echoed by
                                          // the server, which means that you
                                          // won't hear yourself speak. which is
                                          // unfortunate when it comes to demos.
            { // the small size of various voip packets means that the 12 bytes
              // of angles overhead is going to give a noticable size
              // overhead...
                byte b;
                unsigned short s;
        ...
        {
            if(cl_voip_test.value)
                S_Voip_Decode(localplayeridx, s_voip.enccodec,
                    s_voip.generation & 0x0f, initseq, outpos, outbuf);

            // update our own lastspoke, so queries shows that we're speaking
            // when we're speaking in a generic way, even if we can't hear
            // ourselves. but don't update general lastspoke, so ducking applies
            // only when others speak. use capturingvol for yourself. they're
    ...
    if(encpos)
    {
        memmove(s_voip.capturebuf, s_voip.capturebuf + encpos,
            s_voip.capturepos - encpos);
        s_voip.capturepos -= encpos;
    }
}
void S_Voip_Ignore(unsigned int slot, bool ignore)
{
    MSG_WriteByte(&cls.message, clc_stringcmd);
    MSG_WriteString(&cls.message, va("vignore %i %i", slot, ignore));
}
static void S_Voip_Enable_f()
{
    s_voip.voipsendbutton = true;
}
static void S_Voip_Disable_f()
{
    s_voip.voipsendbutton = false;
}
static void S_Voip_f()
{
    if(!strcmp(Cmd_Argv(1), "maxgain"))
    {
        ...
        if(s_voip.speexdsp.preproc)
            qspeex_preprocess_ctl(
                s_voip.speexdsp.preproc, SPEEX_PREPROCESS_SET_AGC_MAX_GAIN, &i);
#endif
    }
}
static void S_Voip_Play_Callback(cvar_t* var, const char* oldval)
{
    MSG_WriteByte(&cls.message, clc_stringcmd);
...
void S_Voip_MapChange()
{
    s_voip.voipsendbutton =
        false; // release +voip on each map change, because people that leave it
               // enabled the whole time are annoying cunts (buttons get stuck
               // sometimes).
    S_Voip_Play_Callback(&cl_voip_play, "");
}

int S_Voip_Loudness(bool ignorevad)
{
    if(s_voip.voiplevel > 100)
    {
        return 100;
    }
    if(!s_voip.cdriverctx || (!ignorevad && s_voip.dumps)) return -1;
    return s_voip.voiplevel;
}
bool S_Voip_Speaking(unsigned int plno)
{
    if(plno >= MAX_SCOREBOARD) return false;
...
{
    int i;
    for(i = 0; i < MAX_SCOREBOARD; i++) s_voip.deccodec[i] = VOIP_INVALID;
    s_voip.enccodec = VOIP_INVALID;
...
void SV_VoiceReadPacket(client_t* client)
{
    ...
    {
        voice.write++;
        for(j = 0; j < bytes; j++) ring->data[j] = MSG_ReadByte();
    }
    ...
    if(vt == client_voip_t::VT_TEAM && !teamplay.value)
        vt = client_voip_t::VT_ALL;

    /*figure out which team members are meant to receive it*/
    for(j = 0; j < MAX_SCOREBOARD / 8; j++) ring->receiver[j] = 0;
    for(j = 0, cl = svs.clients; j < (unsigned int)svs.maxclients; j++, cl++)
    {
        if(!cl->spawned || !cl->active) continue;

        if(vt == client_voip_t::VT_TEAM)
        {
            if((cl->colors & 0xf) == (client->colors & 0xf))
                continue; // on different teams
        }
        else if(vt == client_voip_t::VT_NONMUTED)
        {
            if(client->voip.mute[j >> 3] & (1 << (j & 3))) continue;
        }
        else if(vt >= client_voip_t::VT_PLAYERSLOT0)
        {
            if(j != vt - client_voip_t::VT_PLAYERSLOT0) continue;
        }
...
{
    client->voip.target = client_voip_t::VT_TEAM;
...
void SV_VoiceSendPacket(client_t* client, sizebuf_t* buf)
{
    ...
    while(client->voip.read < voice.write)
    {
        ...
        send = false;
        if(ring->receiver[clno >> 3] & (1 << (clno & 3))) send = true;

        if(client->voip.mute[ring->sender >> 3] & (1 << (ring->sender & 3)))
            send = false;

        if(ring->sender == clno && !sv_voip_echo.value) send = false;

        if(send)
        {
            if(buf->maxsize - buf->cursize < (int)ring->datalen + 5) break;
...
static void SV_Voice_Ignore_f()
{
    ...
    else
    {
        ...
        if(other >= MAX_SCOREBOARD) return;
        ...
        switch(type)
        {
            ...
            case 0:
                host_client->voip.mute[other >> 3] ^= (1 << (other & 3));
                break;
            case 1: host_client->voip.mute[other >> 3] |= (1 << (other & 3));
        }
    }
}
static void SV_Voice_Target_f()
{
    if(cmd_source != src_client)
    {
        if(cls.state == ca_connected) Cmd_ForwardToServer();
    ...
    else
    {
        ...
        if(!strcmp(t, "team"))
        {
            host_client->voip.target = client_voip_t::VT_TEAM;
        }
        else if(!strcmp(t, "all"))
        {
            host_client->voip.target = client_voip_t::VT_ALL;
        }
        else if(!strcmp(t, "nonmuted"))
        {
            host_client->voip.target = client_voip_t::VT_NONMUTED;
        }
        else if(*t >= '0' && *t <= '9')
        {
            other = atoi(t);
            if(other >= MAX_SCOREBOARD) return;
            host_client->voip.target = client_voip_t::VoipTarget(
                client_voip_t::VT_PLAYERSLOT0 + other);
        }
        else
        {
...
static void SV_Voice_MuteAll_f()
{
    if(cmd_source != src_client)
    {
...
static void SV_Voice_UnmuteAll_f()
{
    if(cmd_source != src_client)
    {
