...
static sys_socket_t net_acceptsocket4 =
...
static sys_socket_t net_acceptsocket6 =
    INVALID_SOCKET; // socket for fielding new connections
...
sys_socket_t UDP4_Init()
{
    ...
    if(gethostname(buff, MAXHOSTNAMELEN) != 0)
    {
        err = SOCKETERRNO;
        Con_SafePrintf(
            "UDP4_Init: gethostname failed (%s)\n", socketerror(err));
    }
    else
    {
        ...
        {
            Con_SafePrintf(
                "UDP4_Init: gethostbyname failed (%s)\n", hstrerror(h_errno));
        }
...
sys_socket_t UDP4_Listen(bool state)
{
    if(state)
    {
        // enable listening
...
sys_socket_t UDP4_CheckNewConnections()
{
    int available;
...
int UDP_Write(sys_socket_t socketid, byte* buf, int len, struct qsockaddr* addr)
{
    ...
    else
    {
        ...
        return -1; // some kind of error. a few systems get pissy if the size
                   // doesn't exactly match the address family
    }
...
const char* UDP_AddrToString(struct qsockaddr* addr, bool masked)
{
    ...
    if(addr->qsa_family == AF_INET)
    {
        ...
        q_snprintf(buffer, sizeof(buffer), "%d.%d.%d.%d:%d",
            (haddr >> 24) & 0xff, (haddr >> 16) & 0xff, (haddr >> 8) & 0xff,
    ...
    else if(addr->qsa_family == AF_INET6)
    {
        // evil type punning.
        unsigned short* s =
            (unsigned short*)&((struct sockaddr_in6*)addr)->sin6_addr;
        if(((struct sockaddr_in6*)addr)->sin6_scope_id)
        {
            q_snprintf(buffer, sizeof(buffer),
                "[%x:%x:%x:%x:%x:%x:%x:%x%%%i]:%d", ntohs(s[0]), ntohs(s[1]),
                ntohs(s[2]), ntohs(s[3]), ntohs(s[4]), ntohs(s[5]), ntohs(s[6]),
                ntohs(s[7]), (int)((struct sockaddr_in6*)addr)->sin6_scope_id,
...
sys_socket_t UDP6_Listen(bool state)
{
    if(state)
    {
        // enable listening
...
sys_socket_t UDP6_CheckNewConnections()
{
    int available;
    struct sockaddr_in from;
    socklen_t fromlen;
...
#ifdef __linux__ // sadly there is no posix standard for querying all ipv4+ipv6
                 // addresses.
#include <ifaddrs.h>
...
#else
// for other systems, like macs, where we don't know how to query this stuff
// properly.
