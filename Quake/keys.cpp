...
// QC key codes are based upon DP's keycode constants. This is on account of
// menu.dat coming first.
int Key_NativeToQC(int code)
{
    switch(code)
    {
        ...
        default:
            // ascii chars are mapped as-is (yes this means upper-case keys
            // don't get used).
            if(code >= 0 && code < 127)
            {
                return code;
    ...
    switch(code)
    {
        ...
        default:
            ...
            else if(code < 0)
            {
                ...
                return code; // qc doesn't have extended keys available to it.
                             // so map negative keys back to native ones.
            }
            else
            {
                return -code; // this qc keycode has no native equivelent. use
            }
            // negatives, because we can.
    }
}
...
void Key_Console(int key, const bool fromVirtualKeyboard)
{
    ...
    switch(key)
    {
        ...
        case K_RIGHTARROW:
            ...
            {
                len = strlen(
                    key_lines[(edit_line + (CMDLINES - 1)) & (CMDLINES - 1)]);
                if((int)len <= key_linepos)
                {
                    return; // no character to get
                }
                workline += key_linepos;
                *workline = key_lines[(edit_line + (CMDLINES - 1)) &
                                      (CMDLINES - 1)][key_linepos];
                workline[1] = 0;
                key_linepos++;
            }
            else
...
void Key_SetBinding(int keynum, const char* binding, int bindmap)
{
    ...
    if(keybindings[bindmap][keynum])
    {
        ...
        keybindings[bindmap][keynum] = nullptr;
    }
...
void Key_Unbind_f()
{
    ...
    Key_SetBinding(b, nullptr, bindmap);
}
...
void Key_Event(int key, bool down)
{
    ...
    if(key == K_ESCAPE)
    {
        ...
        switch(key_dest)
        {
            ...
            case key_console:
                ...
                { // Spike -- CSQC needs to be able to intercept
                    // escape. Note that shift+escape will always give
                    // the console for buggy mods.
                    break;
                }
                M_ToggleMenu_f();
                break;
    ...
    if(!down)
    {
        ...
        if(!kb)
        {
            kb = keybindings[key_bindmap[1]]
                            [key]; // FIXME: if the qc changes the bindmap while
        }
        // a key is held then things will break. this
        // is consistent with DP.
        if(kb && kb[0] == '+')
        {
            sprintf(cmd, "-%s %i\n", kb + 1, key);
            Cbuf_AddText(cmd);
