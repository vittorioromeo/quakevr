...
// our servers might as well claim to be 'FTE-Quake' servers. this means FTE can
// see us, we can see FTE (when its pretending to be nq). we additionally look
// for 'DarkPlaces-Quake' servers too, because we can, but most of those servers
// will be using dpp7 and will (safely) not respond to our ccreq_server_info
// requests. we are not visible to DarkPlaces users - dp does not support
...
cvar_t net_masters[] = {{"net_master1", ""}, {"net_master2", ""},
    ...
    {"net_masterextra3", "dpmaster.tchr.no:27950"}, {nullptr}};
...
// QSS
static double
    heartbeat_time; // when this is reached, send a heartbeat to all masters.
...
int Datagram_SendMessage(qsocket_t* sock, sizebuf_t* data)
{
    ...
    // QSS
    sock->max_datagram =
        sock->pending_max_datagram; // this can apply only at the start of a
                                    // reliable, to avoid issues with acks if
                                    // its resized later.
...
static int SendMessageNext(qsocket_t* sock)
{
    ...
    if(sock->sendMessageLength <= sock->max_datagram) // QSS
    {
...
static void _Datagram_ServerControlPacket(sys_socket_t acceptsock,
    struct qsockaddr* clientaddr, byte* data, unsigned int length);
bool Datagram_ProcessPacket(unsigned int length, qsocket_t* sock)
{
    ...
    if(flags & NETFLAG_UNRELIABLE)
    {
        ...
        { // is this even possible? maybe it will be in the future! either way,
          // no sys_errors please.
    ...
    if(flags & NETFLAG_ACK)
    {
        ...
        {
            memmove(sock->sendMessage, sock->sendMessage + sock->max_datagram,
    ...
    if(flags & NETFLAG_DATA)
    {
        ...
        sfunc.Write(
            sock->socket, (byte*)&packetBuffer, NET_HEADERSIZE, &sock->addr);
        ...
        {
            if(sock->receiveMessageLength + length >
                (unsigned int)net_message.maxsize)
            {
...
qsocket_t* Datagram_GetAnyMessage()
{
        ...
        net_landriverlevel++)
    {
        ...
        {
            length =
                dfunc.Read(sock, (byte*)&packetBuffer, NET_DATAGRAMSIZE, &addr);
            if(length == -1 || !length)
            {
            ...
            if(BigLong(packetBuffer.length) & NETFLAG_CTL)
            {
                _Datagram_ServerControlPacket(
                    sock, &addr, (byte*)&packetBuffer, length);
                continue;
            }
    ...
    for(s = net_activeSockets; s; s = s->next)
    {
        ...
        { // timed out, kick them
            // FIXME: add a proper challenge rather than assuming spoofers won't
            // fake acks
            int i;
...
int Datagram_GetMessage(qsocket_t* sock)
{
    ...
    while(true)
    {
        ...
        if(flags & NETFLAG_ACK)
        {
            ...
            sock->sendMessageLength -= sock->max_datagram; // QSS
            if(sock->sendMessageLength > 0)
            {
                memmove(sock->sendMessage,
                    sock->sendMessage + sock->max_datagram,
                    sock->sendMessageLength); // QSS
                sock->sendNext = true;
            }
            else
            {
...
static void Test_f()
{
    const char* host;
    size_t n;                         // QSS
    size_t maxusers = MAX_SCOREBOARD; // QSS
...
int Datagram_Init()
{
    ...
    for(i = 0; i < net_numlandrivers; i++)
    {
        ...
        net_landrivers[i].listeningSock = INVALID_SOCKET; // QSS
        num_inited++;
    }
...
#ifdef BAN_TEST
    Cmd_AddCommand_ClientCommand("ban", NET_Ban_f); // QSS
#endif
...
void Datagram_Rcon_Flush(const char* text)
{
    ...
    *((int*)msg.data) =
    ...
    net_landrivers[rcon_response_landriver].Write(
        rcon_response_socket, msg.data, msg.cursize, &rcon_response_address);
}
// ---
    ...
    struct qsockaddr* clientaddr, byte* data, unsigned int length)
{
    ...
    if(control == -1)
    {
        ...
        { // master, as well as other clients, may send us one of these two
          // packets to get our serverinfo data
            // masters only really need gamename and player counts. actual
            // clients might want player names too.
            ...
            if(*com_token) // the master server needs this. This tells the
                           // master which game we should be listed as.
            {
            ...
            {
                MSG_WriteString(
                    &net_message, va("\\deathmatch\\%s", deathmatch.string));
                net_message.cursize--;
            }
            if(*teamplay.string)
            {
                MSG_WriteString(
                    &net_message, va("\\teamplay\\%s", teamplay.string));
                net_message.cursize--;
            }
            if(*hostname.string)
            {
                MSG_WriteString(
                    &net_message, va("\\hostname\\%s", hostname.string));
                net_message.cursize--;
            }
            ...
            MSG_WriteString(
                &net_message, va("\\sv_maxclients\\%i", svs.maxclients));
            net_message.cursize--;
            if(*cookie)
            {
            ...
            if(full)
            {
                for(i = 0; i < svs.maxclients; i++)
                {
                    if(svs.clients[i].active)
                    {
                        ...
                        MSG_WriteString(&net_message,
                            va("\n%i %i %i_%i \"%s\"", svs.clients[i].old_frags,
                                (int)total, svs.clients[i].colors & 15,
                                svs.clients[i].colors >> 4,
                                svs.clients[i].name));
                        net_message.cursize--;
                    }
                }
    ...
    {
        const char* password =
            MSG_ReadString(); // FIXME: this really needs crypto
        const char* response;
    ...
    for(s = net_activeSockets; s; s = s->next)
    {
        ...
        if(ret == 0)
        {
            ...
            if(ret == 0 && net_time - s->connecttime < 2.0)
            {
                ...
                if(s->proquake_angle_hack)
                {
                    ...
                    MSG_WriteByte(&net_message,
                        30); // ver 30 should be safe. 34 screws with our
            ...
            // FIXME: ideally we would just switch the connection over and
            // restart it with a serverinfo packet. warning: there might be
            // packets in-flight which might mess up unreliable sequences. so we
            // attempt to ignore the request, and let the user restart.
            // FIXME: if this is an issue, it should be possible to reuse the
            // previous connection's outgoing unreliable sequence. reliables
            // should be less of an issue as stray ones will be ignored anyway.
            // FIXME: needs challenges, so that other clients can't determine
            // ip's and spoof a reconnect.
            for(i = 0; i < svs.maxclients; i++)
            {
                if(svs.clients[i].netconnection == s)
                {
                    NET_Close(s); // close early, to avoid svc_disconnects
                                  // confusing things.
                    host_client = &svs.clients[i];
                    SV_DropClient(false);
                    break;
    ...
    {
        MSG_WriteByte(&net_message, 1);  // proquake
        MSG_WriteByte(&net_message, 30); // ver 30 should be safe. 34 screws
                                         // with our single-server-socket stuff.
    ...
    *((int*)net_message.data) =
    ...
    // spawn the client.
    // FIXME: come up with some challenge mechanism so that we don't go to the
    // expense of spamming serverinfos+modellists+etc until we know that its an
    ...
    if(sv_public.value > 0)
    {
        if(Sys_DoubleTime() > heartbeat_time)
        {
            ...
            for(k = 0; net_masters[k].string; k++)
            {
                ...
                {
                    if(net_landrivers[net_landriverlevel].initialized &&
                        dfunc.listeningSock != INVALID_SOCKET)
                    {
...
static void _Datagram_AddPossibleHost(struct qsockaddr* addr, bool master)
{
    ...
    {
        if(!memcmp(&hostlist[u].addr, addr, sizeof(struct qsockaddr)) &&
            hostlist[u].driver == net_landriverlevel)
        { // we already know about it. it must have come from some other master.
          // don't respam.
            return;
        }
    }
...
static void Info_ReadKey(
    const char* info, const char* key, char* out, size_t outsize)
{
               ...
               net_message.maxsize, &readaddr)) > 0)
    {
        ...
        if(control == -1)
        {
            if(msg_readcount + 19 <= net_message.cursize &&
                !strncmp((char*)net_message.data + msg_readcount,
                    "getserversResponse", 18))
            {
                struct qsockaddr addr;
                int i;
                msg_readcount += 18;
            ...
            else if(msg_readcount + 13 <= net_message.cursize &&
                    !strncmp((char*)net_message.data + msg_readcount,
                        "infoResponse\n", 13))
            { // response from a dpp7 server (or possibly 15, no idea really)
                ...
                Info_ReadKey(info, "hostname", hostcache[n].name,
                    sizeof(hostcache[n].name));
                Info_ReadKey(info, "mapname", hostcache[n].map,
                    sizeof(hostcache[n].map));
                Info_ReadKey(info, "modname", tmp, sizeof(tmp));
                if(!COM_GameDirMatches(tmp))
                {
                    q_strlcpy(hostcache[n].name, va("{%s}", tmp),
                        sizeof(hostcache[n].name));
                ...
                Q_memcpy(
                    &hostcache[n].addr, &readaddr, sizeof(struct qsockaddr));
                hostcache[n].driver = net_driverlevel;
                hostcache[n].ldriver = net_landriverlevel;
                q_strlcpy(hostcache[n].cname,
                    dfunc.AddrToString(&readaddr, false),
        ...
        q_strlcpy(
            hostcache[n].name, MSG_ReadString(), sizeof(hostcache[n].name));
        ...
        hostcache[n].driver = net_driverlevel;
        hostcache[n].ldriver = net_landriverlevel;
        q_strlcpy(hostcache[n].cname, dfunc.AddrToString(&readaddr, false),
            sizeof(hostcache[n].cname));
...
static qsocket_t* _Datagram_Connect(struct qsockaddr* serveraddr)
{
    ...
    for(reps = 0; reps < 3; reps++)
    {
        ...
        MSG_WriteByte(&net_message, NET_PROTOCOL_VERSION);
        if(sock->proquake_angle_hack)
        { /*Spike -- proquake compat. if both engines claim to be using mod==1
             then 16bit client->server angles can be used. server->client angles
             remain 16bit*/
            ...
            MSG_WriteLong(&net_message,
                0); // strtoul(password.string, NULL, 0)); /*password*/
        }
        *((int*)net_message.data) =
            BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
...
// if the (DP) server is running using vanilla protocols, it replies to the
// above, otherwise to the following, requiring both to be sent. (challenges
// hinder a DOS issue known as smurfing, in that the client must prove that it
// owns the IP that it might be spoofing before any serious resources are used)
#define DPGETCHALLENGE "\xff\xff\xff\xffgetchallenge\n"
        dfunc.Write(
            newsock, (byte*)DPGETCHALLENGE, strlen(DPGETCHALLENGE), serveraddr);
            ...
            if(ret > 0)
            {
                ...
                if(dfunc.AddrCompare(&readaddr, serveraddr) != 0)
                {
                    ...
                    Con_SafePrintf("Expected: %s | %s\n",
                        dfunc.AddrToString(serveraddr, false),
                        StrAddr(serveraddr));
                ...
                if(control == -1)
                {
                    ...
                    { // either a q2 or dp server...
                        ...
                        q_snprintf(buf, sizeof(buf),
                            "%c%c%c%cconnect\\protocol\\darkplaces "
                            "3\\protocols\\RMQ FITZ DP7 NEHAHRABJP3 "
                            "QUAKE\\challenge\\%s",
                            255, 255, 255, 255, s + 10);
                        dfunc.Write(
                            newsock, (byte*)buf, strlen(buf), serveraddr);
                    }
                    else if(!strcmp(s, "accept"))
                    {
                        Q_memcpy(
                            &sock->addr, serveraddr, sizeof(struct qsockaddr));
                    ...
                    /*else if (!strcmp(s, "reject"))
                    {
                        ...
                        Con_Printf("%s\n", reason);
                        q_strlcpy(m_return_reason, reason,
                    sizeof(m_return_reason)); goto ErrorReturn;
                    }*/
...
dpserveraccepted:
    ...
    The server is now about(or already did, yay resends) to send us a packet
    from its data port to our address. The nat will (correctly) see a packet
    from a different remote address:port. The local nat has two options here. 1)
    assume that the wrong port is fine. 2) drop it. Dropping it is far more
    likely. The NQ code will not send any unreliables until we have received the
    serverinfo. There are no reliables that need to be sent either. Normally we
    won't send ANYTHING until we get that packet. Which will never happen
    because the NAT will never let it through. So, if we want to get away
    without fixing everyone else's server (which is also quite messy), the easy
    way around this dilema is to just send some (small) useless packet to what
    we believe to be the server's data port. A single unreliable clc_nop should
    do it. There's unlikely to be much packetloss on our local lan (so long as
    our host buffers outgoing packets on a per-socket basis or something), so we
    don't normally need to resend. We don't really care if the server can even
    read it properly, but its best to avoid warning prints. With that small
    outgoing packet, our local nat will think we initiated the request.
    HOPEFULLY it'll reuse the same public port+address. Most home routers will,
    but not all, most hole-punching techniques depend upon such behaviour. Note
    that proquake 3.4+ will actually wait for a packet from the new client,
    which solves that (but makes the nop mandatory, so needs to be reliable).

    the nop is actually sent inside CL_EstablishConnection where it has cleaner
    access to the client's pending reliable message.

    Note that we do fix our own server. This means that we can easily run on a
    home nat. the heartbeats to the master will open up a public port with most
    routers. And if that doesn't work, then its easy enough to port-forward a
    single known port instead of having to DMZ the entire network. I don't
    really expect that many people will use this, but it'll be nice for the
    occasional coop game. (note that this makes the nop redundant, but that's a
...
qsocket_t* Datagram_Connect(const char* host)
{
        ...
        net_landriverlevel++)
    {
        ...
        {
            // see if we can resolve the host name
            // Spike -- moved name resolution to here to avoid extraneous 'could
            // not resolves' when using other address families
...
/*
Spike: added this to list more than one ipv4 address (many people are still
multi-homed)
*/
