...
void ED_PrintEdicts()
{
    ...
    for(i = 0; i < qcvm->num_edicts; i++) ED_PrintNum(i);
    PR_SwitchQCVM(NULL);
}
...
bool ED_ParseEpair(void* base, ddef_t* key, const char* s)
{
    ...
    switch(key->type & ~DEF_SAVEGLOBAL)
    {
        ...
        case ev_entity:
            if(!strncmp(
                   s, "entity ", 7)) // Spike: putentityfieldstring/etc should
...
{
    if(qcvm && nvm) Sys_Error("PR_SwitchQCVM: A qcvm was already active");
    qcvm = nvm;
    if(qcvm)
...
{
    qcvm_t* oldvm = qcvm;
    if(!vm->progs) return; // wasn't loaded.
    qcvm = NULL;
    PR_SwitchQCVM(vm);
    PR_ShutdownExtensions();

    if(qcvm->knownstrings) Z_Free((void*)qcvm->knownstrings);
...
===============
*/
qboolean PR_LoadProgs(const char* filename, qboolean fatal, builtin_t* builtins,
    size_t numbuiltins)
{
    int i;
    unsigned int u;
    ...
    if(qcvm->extfields.emiteffectnum < 0) qcvm->extfields.emiteffectnum = i++;
    if(qcvm->extfields.traileffectnum < 0) qcvm->extfields.traileffectnum = i++;
...
int NUM_FOR_EDICT(edict_t* e)
{
    ...
    if(b < 0 || b >= qcvm->num_edicts) Host_Error("NUM_FOR_EDICT: bad pointer");
    return b;
}
...
void PR_ClearEngineString(int num)
{
    if(num < 0 && num >= -qcvm->numknownstrings)
    {
        ...
        if(qcvm->freeknownstrings > num) qcvm->freeknownstrings = num;
    }
}
...
int PR_SetEngineString(const char* s)
{
    ...
    {
        if(qcvm->knownstrings[i] == s) return -1 - i;
    }
    ...
    for(i = qcvm->freeknownstrings;; i++)
    {
        ...
        {
            if(qcvm->knownstrings[i]) continue;
        }
        else
        {
            if(i >= qcvm->maxknownstrings) PR_AllocStringSlots();
...
int PR_AllocString(int size, char** ptr)
{
    ...
    if(ptr) *ptr = (char*)qcvm->knownstrings[i];
    return -1 - i;
}
