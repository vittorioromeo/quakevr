...
static int PR_EnterFunction(dfunction_t* f)
{
    ...
    qcvm->stack[qcvm->depth].f = qcvm->xfunction;
    qcvm->depth++;
    if(qcvm->depth >= MAX_STACK_DEPTH) PR_RunError("stack overflow");
    ...
    for(i = 0; i < f->numparms; i++)
    {
        ...
        {
            ((int*)qcvm->globals)[o] =
                ((int*)qcvm->globals)[OFS_PARM0 + i * 3 + j];
            o++;
        }
    }
...
static int PR_LeaveFunction(void)
{
    ...
    if(qcvm->depth <= 0) Host_Error("prog stack underflow");
...
void PR_ExecuteProgram(func_t fnum)
{
    ...
    while(1)
    {
        ...
        if(qcvm->trace) PR_PrintStatement(st);
        ...
        switch(st->op)
        {
            ...
            case OP_RETURN:
                ...
                startprofile = profile;
                qcvm->xstatement = st - qcvm->statements;
                qcvm->globals[OFS_RETURN] =
                    qcvm->globals[(unsigned short)st->a];
