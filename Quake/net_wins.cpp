...
// ipv4 defs
static sys_socket_t netv4_acceptsocket =
...
static in_addr_t myAddrv4,
    bindAddrv4; // spike --keeping separate bind and detected values.

// ipv6 defs
#ifdef IPPROTO_IPV6
typedef struct in_addr6 in_addr6_t;
static sys_socket_t netv6_acceptsocket =
    INVALID_SOCKET; // socket for fielding new connections
...
// we don't use hybrid sockets, so things are a little easier when it comes to
// xp vs vista. we don't detect the win2k ipv6 tech preview thing. it has
// different sized addresses, so lets hope microsoft's code handles that if it
// ever comes up.
int(WSAAPI* qgetaddrinfo)(const char* nodename, const char* servname,
...
static void WINIPv4_GetLocalAddress()
{
    ...
    addr = ntohl(myAddrv4);
    sprintf(my_ipv4_address, "%ld.%ld.%ld.%ld", (addr >> 24) & 0xff,
...
{
    unsigned char* ptr =
        ((struct sockaddr_in6*)addr)->sin6_addr.s6_addr + wordnum * 2;
    ...
    if(addr->qsa_family == AF_INET6)
    {
        if(masked)
        {
            q_snprintf(buffer, sizeof(buffer), "[%x:%x:%x:%x::]/64",
                ntohs_v6word(addr, 0), ntohs_v6word(addr, 1),
                ntohs_v6word(addr, 2), ntohs_v6word(addr, 3));
        }
        else
        {
            if(((struct sockaddr_in6*)addr)->sin6_scope_id)
            {
                q_snprintf(buffer, sizeof(buffer),
                    "[%x:%x:%x:%x:%x:%x:%x:%x%%%i]:%d", ntohs_v6word(addr, 0),
                    ntohs_v6word(addr, 1), ntohs_v6word(addr, 2),
                    ntohs_v6word(addr, 3), ntohs_v6word(addr, 4),
                    ntohs_v6word(addr, 5), ntohs_v6word(addr, 6),
                    ntohs_v6word(addr, 7),
            ...
            {
                q_snprintf(buffer, sizeof(buffer),
                    "[%x:%x:%x:%x:%x:%x:%x:%x]:%d", ntohs_v6word(addr, 0),
                    ntohs_v6word(addr, 1), ntohs_v6word(addr, 2),
                    ntohs_v6word(addr, 3), ntohs_v6word(addr, 4),
                    ntohs_v6word(addr, 5), ntohs_v6word(addr, 6),
                    ntohs_v6word(addr, 7),
...
int WINIPv4_GetAddresses(qhostaddr_t* addresses, int maxaddresses)
{
    ...
    {
        // on windows, we can just do a dns lookup on our own hostname and
        // expect an ipv4 result
        char hostname[64];
        u_long addr;
        ...
        if(h && h->h_addrtype == AF_INET)
        {
            for(b = 0; h->h_addr_list[b] && result < maxaddresses; b++)
            {
                ...
                q_snprintf(addresses[result++], sizeof(addresses[0]),
                    "%ld.%ld.%ld.%ld", (addr >> 24) & 0xff, (addr >> 16) & 0xff,
...
static void WINIPv6_GetLocalAddress()
{
    ...
    if(qgetaddrinfo && qgetaddrinfo(buff, nullptr, &hints, &local) == 0)
    {
        ...
        q_strlcpy(my_ipv6_address,
            WINS_AddrToString((struct qsockaddr*)local->ai_addr, false),
            sizeof(my_ipv6_address));
...
sys_socket_t WINIPv6_Init()
{
    ...
    if(gethostname(buff, MAXHOSTNAMELEN) != 0)
    {
        int err = SOCKETERRNO;
        Con_SafePrintf(
            "WINIPv6_Init: gethostname failed (%s)\n", socketerror(err));
    }
    else
    {
        buff[MAXHOSTNAMELEN - 1] = 0;
    ...
    if(i)
    {
        if(i < com_argc - 1)
        {
            if(WINIPv6_GetAddrFromName(
                   com_argv[i + 1], (struct qsockaddr*)&bindAddrv6))
                Sys_Error("%s is not a valid IPv6 address", com_argv[i + 1]);
            if(!*my_ipv6_address) strcpy(my_ipv6_address, com_argv[i + 1]);
        }
        else
        {
    ...
    {
        Con_SafePrintf(
            "WINIPv6_Init: Unable to open control socket, UDP disabled\n");
...
sys_socket_t WINIPv6_OpenSocket(int port)
{
    ...
    setsockopt(
        newsocket, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&_true, sizeof(_true));
    ...
    if(bind(newsocket, (struct sockaddr*)&address, sizeof(address)) == 0)
    {
        ...
        setsockopt(
            newsocket, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char*)&req, sizeof(req));
...
int WINIPv6_GetAddrFromName(const char* name, struct qsockaddr* addr)
{
    ...
    else
    {
        ...
        if(port)
        {
            ...
            dupbase[len] = '\0';
            error = qgetaddrinfo
                        ? qgetaddrinfo(dupbase, port + 1, &udp6hint, &addrinfo)
                        : EAI_NONAME;
        }
        else
            error = EAI_NONAME;
