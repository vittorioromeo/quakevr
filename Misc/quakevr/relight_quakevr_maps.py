...
from relight_maps import entities_text, find_light, glow_lights, pak_file, with_entities
...
def relight(name, settings, light, maps, out_dir, palette):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with open(work, "wb") as f:
            f.write(with_entities(data, ents + extra))
        result = subprocess.run([light] + settings["args"] + [work], cwd=tmp, stdout=subprocess.PIPE,
                                stderr=subprocess.STDOUT, text=True, errors="replace")
        if result.returncode != 0:
            print(result.stdout[-3000:])
            sys.exit("%s: light failed" % name)
        with open(work, "rb") as f:
            relit = with_entities(f.read(), bsp_ents)
        lit = os.path.join(tmp, name + ".lit")
        lit_data = None
        if os.path.isfile(lit):
            if not settings["lit"]:
                sys.exit("%s: light made a .lit: the lights are meant to be white" % name)
            with open(lit, "rb") as f:
                lit_data = f.read()
        elif settings["lit"]:
...
def main():
    ...
    for name in args.only or sorted(MAPS):
        relight(name, MAPS[name], light, args.maps, args.out or args.maps, palette)
