...
| P5 Input & haptics | 🟡 part 1 (untested on HMD) | OpenXR actions (fire, grab, reload, next weapon, jump, menu, move, turn, haptics) for simple/Touch/Touch Plus/Index/Vive/WMR; stick locomotion, snap/smooth turning, menu navigation; networked `haptic`. Hand/head velocities from the runtime; throw estimator (`vr_throw.cpp`) and true-scale throws (see TESTING.md). Optional sticky grip (`vr_weapon_grip_mode 1`, issue #31). Remaining: teleport, 2H aiming, finger tracking, holster hotspots, body yaw from the hands, flick reload, bindable controller keys (issue #12). |
